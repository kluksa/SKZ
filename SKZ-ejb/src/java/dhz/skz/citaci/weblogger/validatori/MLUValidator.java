...
import dhz.skz.citaci.weblogger.util.GreskaBitovi;
...
public class MLUValidator implements Validator {
    ...
    public void validiraj(PodatakSirovi ps) {
        ...
        int fs = Integer.parseInt(st[2]);
        int nc = Integer.parseInt(st[3]);
        int greska = ps.getGreska();
        
        if (fs != 0) {
            greska |= GreskaBitovi.FAULT.ordinal();
        }
        if (ss != 0) {
            greska |= GreskaBitovi.FAULT.ordinal();
        }
        if (bs == 0) {
            ps.setStatus(0);
            ps.getProgramMjerenjaId().getMetodaId().getPonovljivostUNuli();
        } else {
            if ((bs & 1) == 1) {
