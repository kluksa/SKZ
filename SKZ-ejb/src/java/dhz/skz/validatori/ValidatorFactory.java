...
public class ValidatorFactory {
    ...
    private HashMap<ProgramMjerenja, NavigableMap<Date, Validator>> validatori;
    ...
    public Validator getValidator(ProgramMjerenja pm, Date vrijeme) {
        return validatori.get(pm).floorEntry(vrijeme).getValue();
    }
...
//            validatori.put(pul.getVrijemePostavljanja(),lookupValidatorBean);
    ...
    public void init(IzvorPodataka ip) throws NamingException {
        NavigableMap<Date, Validator> val = new TreeMap<>();
        validatori = new HashMap<>();
    }
}
